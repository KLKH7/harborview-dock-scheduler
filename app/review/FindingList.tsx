  ...
  return (
    <div className="flex w-full min-w-0 flex-col border-r border-line sm:w-[440px] sm:shrink-0">
      <div className="flex h-10 shrink-0 items-center gap-3 border-b border-line px-4">
        <h1 className="text-[18px] font-medium tracking-tight text-ink">Review</h1>
        <span className="tnum text-[13px] text-mute">{todoCount} to review</span>
        {dismissedCount > 0 && (
          <Link
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {lever.of > 0 && lever.clears > 0 && (
          <p className="tnum border-b border-line px-4 py-2.5 text-[13px] text-mute">
            Setting a length for {lever.vessels} vessel{lever.vessels === 1 ? '' : 's'} would settle {lever.clears} of the {lever.of} oversize stays.
          </p>
        )}

        {rows.length === 0 ? (
          <p className="border-b border-line px-4 py-3 text-[14px] text-mute">
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className={`truncate text-[14px] text-ink ${disp ? 'line-through' : ''}`}>{r.title}</div>
        <div className="tnum truncate text-[13px] text-mute">
          {KIND_LABEL[r.kind]} · {fmtShort(r.when)}
          {disp && ` · ${disp.status === 'data_error' ? 'data error' : disp.status}`}
        </div>
      </div>
      <span className="tnum shrink-0 self-start text-[13px] text-mute">{relativeDays(r.when, today)}</span>
    </Link>
  )
}
