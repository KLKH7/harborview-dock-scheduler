    ...
    return (
      <div className="hidden min-h-0 flex-1 flex-col sm:flex">
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="tnum max-w-[36ch] text-[14px] text-mute">
            {todoCount === 0
              ? 'Nothing to review.'
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
      <div className="mx-auto w-full max-w-[720px] px-6 py-6">
        <p className="text-[11px] uppercase tracking-[0.06em] text-mute">
          {row.kind === 'overlap' ? 'Two stays in one berth' : row.kind === 'cross' ? 'One vessel in two berths' : 'Too long for the berth'}
        </p>
        <h2 className="mt-1 text-[18px] font-medium tracking-tight text-ink">{row.title}</h2>
        <p className="tnum mt-1 text-[14px] text-mute">{row.detail}</p>

        <div className="mt-5 divide-y divide-line rounded-[8px] border border-line bg-panel">
          {row.stays.slice(0, 8).map((s) => (
            <div key={s.id} className="flex items-baseline gap-3 px-4 py-2.5 text-[14px]">
          ...
          {row.stays.length > 8 && (
            <p className="tnum px-4 py-2 text-[13px] text-mute">and {row.stays.length - 8} more</p>
          )}
        </div>

        {row.kind === 'oversize' && row.vessel && (
          <div className="mt-5">
            <p className="text-[11px] uppercase tracking-[0.06em] text-mute">Length on roster</p>
            <div className="mt-1.5">
              <PairLengthFix vesselId={row.vessel.id} currentFt={row.vessel.lengthFt ?? 0} berthFt={row.vessel.berthFt} />
            </div>
            <p className="tnum mt-1.5 text-[13px] text-mute">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded border border-line bg-panel px-2.5 py-1.5 text-[14px] text-ink hover:bg-wash"
          >
        ...
        <div className="mt-6 border-t border-line pt-5">
          <p className="text-[11px] uppercase tracking-[0.06em] text-mute">Decision</p>
          {disposition ? (
            <div className="mt-2 text-[14px]">
              ...
              <button type="button" disabled={pending} onClick={() => act(null)} className="mt-2 text-[13px] text-mute hover:text-ink">
                Reopen
              </button>
            </div>
          ) : (
            <div className="mt-2 space-y-2">
              <input
                ...
                className="w-full rounded border border-line bg-panel px-2.5 py-1.5 text-[14px] text-ink outline-none focus:border-ink"
              />
              <div className="flex flex-wrap gap-2">
                <button type="button" disabled={pending} onClick={() => act('accepted')} className="rounded bg-ink px-2.5 py-1.5 text-[14px] text-paper disabled:opacity-50">
                  Accept as is
                </button>
                <button type="button" disabled={pending} onClick={() => act('data_error')} className="rounded border border-line bg-panel px-2.5 py-1.5 text-[14px] text-ink hover:bg-wash disabled:opacity-50">
                  Data error
                </button>
              </div>
...
function RecentChanges({ changes }: { changes: ChangeEntry[] }) {
  ...
  return (
    <div className="mt-auto border-t border-line px-6 py-4">
      <p className="text-[11px] uppercase tracking-[0.06em] text-mute">Recent changes</p>
      <ul className="mt-2 space-y-1">
        {changes.slice(0, 8).map((c) => {
          ...
          return (
            <li key={c.id} className="tnum flex gap-3 text-[13px] text-mute">
